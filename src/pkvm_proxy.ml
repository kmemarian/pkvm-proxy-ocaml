...
let init_vm ?(vcpus = 1) ?(protected = true) () =
  ...
  topup_memcache_ptr (Region.memory buf) 10;
  ...
  match hvc (Pkvm_init_vm (host_kvm.kaddr, pgd.kaddr)) with
  | handle ->
      Region.close pgd;
...
let teardown_vm ?free_memcache:(free_mc = true) ?cleanup vm =
  ...
  Option.fold ~none:() ~some:(fun f -> f ()) cleanup;
